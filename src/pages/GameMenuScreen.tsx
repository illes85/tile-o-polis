...
const GameMenuScreen: React.FC = () => {
  ...
  const handleBuildingDesigner = () => {
    navigate("/building-designer");
  };

  const handleGameDesigner = () => {
    navigate("/game-designer");
  };
          ...
          <div className="flex flex-col items-center">
            ...
            <Card className="w-full bg-black/40 backdrop-blur-md border-white/10 shadow-2xl">
              <CardContent className="flex flex-col space-y-3 p-6">
                ...
                <Button 
                  onClick={handleBuildingDesigner} 
                  disabled={isProcessing} 
                  className="w-full h-12 text-lg bg-purple-600/30 hover:bg-purple-600/50 text-white border border-purple-500/30"
                >
                  <Building className="mr-2 h-5 w-5" /> Épülettervező
                </Button>
                <Button 
                  onClick={handleGameDesigner} 
                  disabled={isProcessing} 
                  className="w-full h-12 text-lg bg-cyan-600/30 hover:bg-cyan-600/50 text-white border border-cyan-500/30"
                >
                  <Settings className="mr-2 h-5 w-5" /> Játéktervező
                </Button>
