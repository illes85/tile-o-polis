...
const FORESTRY_HOUSE_MAX_EMPLOYEES = 1; // Módosítva: 1 dolgozó
...
const availableBuildingOptions: BuildingOption[] = [
  {
    ...
    width: 2,
    height: 1,
    rentalPrice: 0, // Nincs bérleti díj
    capacity: 1,
  },
  {
    type: "house",
  ...
  },
  {
    ...
    name: "Farm",
    cost: 1000,
    brickCost: 5, // Tégla költség
    woodCost: 3, // Fa költség
    duration: 10000, // 10 másodperc
    width: 4,
    height: 4,
    salary: 5, // Példa fizetés
    capacity: 2, // Példa kapacitás
  },
];
...
const Game = () => {
  ...
  useEffect(() => {
    ...
    const placeInitialBuilding = (
      buildingId: string,
      buildingType: "house" | "office" | "forestry" | "farm", // Frissítve
      buildingWidth: number,
      buildingHeight: number,
      rentalPrice?: number,
      salary?: number,
  ...
  useEffect(() => {
    const gameTickTimer = setInterval(() => {
      setPlayers(prevPlayers => prevPlayers.map(player => {
        ...
        const playerRentedBuildings = buildings.filter(b => b.renterId === player.id && b.ownerId !== player.id); // Csak a bérelt, de nem saját tulajdonú házakért fizet
        ...
        if (totalRent > 0) {
          if (newMoney >= totalRent) {
            ...
            setBuildings(prevBuildings =>
              prevBuildings.map(b =>
                b.renterId === player.id && b.ownerId !== player.id // Csak a bérelt, de nem saját házat ürítjük ki
  ...
  const handleRentBuilding = () => {
    ...
    if (isOwner) {
      ...
      // Tulajdonos beköltözik, nem fizet bérleti díjat
      setBuildings(prevBuildings =>
        prevBuildings.map(b =>
  ...
  const handleBuildBuilding = (buildingType: "house" | "office" | "forestry" | "farm") => {
    const buildingOption = availableBuildingOptions.find(opt => opt.type === buildingType && opt.name === (buildingType === "house" ? (opt.name === "Sátor" ? "Sátor" : "Házikó") : opt.name)); // Pontosabb keresés a ház típusoknál
            ...
            p.id === currentPlayerId
              ? {
                  ...
                  inventory: {
                    ...
                    brick: p.inventory.brick - (buildingToPlace.brickCost || 0), // Tégla levonása
                  },
                }
              : p
          )
            ...
            <DialogFooter>
              {selectedBuilding.type === "house" && (
                <Button
                  ...
                  disabled={
                    (selectedBuilding.ownerId === currentPlayerId && selectedBuilding.residentIds.includes(currentPlayerId)) || // Saját házban már lakik
                    (!selectedBuilding.ownerId && selectedBuilding.renterId === currentPlayerId) || // Bérelte, de nem saját
