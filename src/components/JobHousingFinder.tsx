...
interface JobHousingFinderProps {
    ...
    onResignFromJob: (buildingId: string) => void;
    onMoveOut: (buildingId: string) => void;
}
...
const JobHousingFinder: React.FC<JobHousingFinderProps> = ({
    ...
    const getBuildingIcon = (type: BuildingData['type']) => {
        switch (type) {
            ...
            case 'quarry': return <Gem className="h-4 w-4 text-gray-500" />;
        ...
        return (
            <Card key={job.id} className="p-3 flex justify-between items-center border-l-4 border-blue-500">
                ...
                {isCurrentWorkplace ? (
                  <Button
                    size="sm"
                    onClick={() => onResignFromJob(job.id)}
                    variant="destructive"
                  >
                    Felmondás
                  </Button>
    ...
    const renderHousingCard = (house: BuildingData) => {
        ...
        const alreadyRenting = isRenting && !isCurrentResidence;
        const isFull = house.residentIds.length >= house.capacity;
        const cannotAfford = !isOwner && currentPlayer.money < (house.rentalPrice || 0);
        ...
        return (
            <Card key={house.id} className={`p-3 flex justify-between items-center border-l-4 ${!cannotAfford ? 'border-green-500' : 'border-red-500'}`}>
                <div className="flex items-center space-x-3">
                ...
                {isCurrentResidence ? (
                    <Button size="sm" variant="destructive" onClick={() => onMoveOut(house.id)}>
                        Kiköltözés
                    </Button>
                ) : (
                    <Button 
