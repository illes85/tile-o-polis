...
import cropTileset from "@/assets/vectoraith_tileset_farming_sim_essentials/48x48/Tilesets (Compact)/vectoraith_tileset_farmingsims_crops_48x48.png";
import detailsTileset from "@/assets/vectoraith_tileset_farming_sim_essentials/32x32/Tilesets (Modular)/vectoraith_tileset_farmingsims_details_summer_32x32.png";
...
const BUILDING_TILESET_48 = buildingTileset48;
const DETAILS_TILESET_48 = detailsTileset48;

const TILESET_MAP: Record<string, string> = {
  "buildings_32": BUILDING_TILESET_32,
  "details_32": DETAILS_TILESET,
  "buildings_48": BUILDING_TILESET_48,
  "details_48": DETAILS_TILESET_48,
};

import { CropType, FarmlandTile, BuildingData } from "@/types/gameTypes";
...
const Building: React.FC<BuildingProps> = ({
  ...
  isDemolishing,
  demolishProgress
}: BuildingProps & { isDemolishing?: boolean; demolishProgress?: number }) => {
  const [isHovered, setIsHovered] = useState(false);
  const [now, setNow] = useState(Date.now());

  // Frissítjük az időt másodpercenként a progressz számításához
  useEffect(() => {
    if ((isUnderConstruction || isDemolishing) && !isGhost) {
      const interval = setInterval(() => setNow(Date.now()), 1000);
      return () => clearInterval(interval);
    }
  }, [isUnderConstruction, isDemolishing, isGhost]);
  ...
  if (isUnderConstruction && constructionEta && originalDuration) {
    ...
    visualClasses = "bg-red-900/80 opacity-90 border border-red-700";
    
    content = (
      <div className="flex flex-col items-center justify-center h-full w-full p-1"> 
        <Trash2 className="h-6 w-6 text-white mb-1 animate-pulse" />
        <span className="text-white text-xs">Bontás...</span>
        <Progress value={demolishProgress} className="w-3/4 h-2 mt-1 bg-red-950" indicatorClassName="bg-red-500" />
        <span className="text-white text-[0.6rem] mt-1">{Math.floor(demolishProgress)}%</span>
      </div>
    );
  } else {
    visualClasses = "rounded-md cursor-pointer transition-colors"; 
    ...
    switch (type) {
      ...
      case "custom":
        visualClasses = "cursor-pointer transition-colors";
        if (customGraphics) {
            let tilesetSrc = TILESET_MAP[customGraphics.tileset];
            
            if (customGraphics.tileset === "custom_url" && customGraphics.customUrl) {
                tilesetSrc = customGraphics.customUrl;
            } else if (customGraphics.tileset === "custom_file" && customGraphics.customBase64) {
                tilesetSrc = customGraphics.customBase64;
            }
            
            if (!tilesetSrc) tilesetSrc = BUILDING_TILESET_32;

            const tileSize = customGraphics.tileSize || 32;
            const bgX = customGraphics.x * tileSize;
            const bgY = customGraphics.y * tileSize;
            
            // Calculate scale to fit the larger/smaller tileset into the cellSizePx grid
            const scale = cellSizePx / tileSize;
            ...
            content = (
                ...
                {isOwnedByPlayer && <Home className="absolute top-1 right-1 h-3 w-3 text-yellow-400 drop-shadow-md" />}
                </>
            );
        } else {
             content = <span className="text-xs p-1">{name}</span>;
        }
        break;
      case "farmland": {
